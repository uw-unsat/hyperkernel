...
static struct inode *namex(struct file *cwd, char *path, int nameiparent, char *name)
{
...
struct inode *namei(struct file *cwd, char *path)
{
    char name[DIRSIZ];
    return namex(cwd, path, 0, name);
}

struct inode *nameiparent(struct file *cwd, char *path, char *name)
{
